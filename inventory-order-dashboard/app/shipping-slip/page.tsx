...
export default function ShippingSlipPage() {
  ...
  return <main className="shipping-slip-page min-h-screen bg-[#F8F7F4] px-5 py-8 text-[#292824] sm:px-8 sm:py-12 print:bg-white print:p-0">
    <div className="shipping-slip-shell mx-auto w-full max-w-[794px]">
      ...
      <section className="shipping-slip-sheet overflow-hidden rounded-2xl border border-[#DDD8D0] bg-white sm:grid sm:aspect-[210/297] sm:grid-rows-2 print:rounded-none print:border-0">
        <div className="shipping-slip-top">
        ...
        <section className="grid gap-px border-b border-[#E8E4DE] bg-[#E8E4DE] sm:grid-cols-[1.25fr_.75fr_.9fr]">
          <div className="bg-white p-5"><p className="text-[10px] font-bold tracking-[.16em] text-[#938D84]">收件人姓名</p><b className="mt-2 block text-2xl leading-tight tracking-[-.04em] text-[#292824]">{customer?.name || "—"}</b><p className="mt-2 text-xs text-[#746E65]">{customer?.phone || "未提供電話"}</p></div>
          <div className="bg-[#FFF8F0] p-5"><p className="text-[10px] font-bold tracking-[.16em] text-[#A16B3B]">付款狀態</p><b className={`mt-2 block text-xl leading-tight ${order.reconciliation_status === "已付款" ? "text-[#477154]" : "text-[#A35F37]"}`}>{order.reconciliation_status === "已付款" ? "已付款" : "需付款"}</b><p className="mt-2 text-xs text-[#746E65]">{order.payment_method}</p></div>
          <div className="bg-[#F2F7F2] p-5"><p className="text-[10px] font-bold tracking-[.16em] text-[#58705E]">付款金額</p><b className="mt-2 block text-2xl leading-tight tracking-[-.04em] text-[#31513A]">{currency(order.total)}</b><p className="mt-2 text-xs text-[#746E65]">{order.reconciliation_status === "已付款" ? "已收款" : "請向客戶收款"}</p></div>
        </section>

        <div className="grid grid-cols-2 gap-5 p-5"><section><p className="text-[10px] font-bold tracking-[.16em] text-[#A09A90]">RECIPIENT</p><h3 className="mt-1 text-sm font-semibold">收件資訊</h3><div className="mt-3 space-y-2.5 text-xs"><p><span className="block text-[10px] text-[#938D84]">LINE@名稱</span><b className="mt-1 block">{customer?.line_name || "—"}</b></p><p><span className="block text-[10px] text-[#938D84]">電話</span><b className="mt-1 block">{customer?.phone || "—"}</b></p><p><span className="block text-[10px] text-[#938D84]">配送地址</span><b className="mt-1 block leading-5">{customer?.address || "—"}</b></p></div></section><section><p className="text-[10px] font-bold tracking-[.16em] text-[#A09A90]">DELIVERY</p><h3 className="mt-1 text-sm font-semibold">配送資訊</h3><div className="mt-3 grid grid-cols-2 gap-3 text-xs"><p><span className="block text-[10px] text-[#938D84]">配送方式</span><b className="mt-1 block">{order.delivery_method}</b></p><p><span className="block text-[10px] text-[#938D84]">付款方式</span><b className="mt-1 block leading-5">{order.payment_method}</b></p><p className="col-span-2"><span className="block text-[10px] text-[#938D84]">出貨備註</span><b className="mt-1 block leading-5">{order.note || "—"}</b></p></div></section></div>
        </div>

        <section className="shipping-slip-bottom flex min-h-0 flex-col border-t-2 border-dashed border-[#B9B1A5]"><div className="shipping-slip-items flex-1 px-5 py-5"><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-bold tracking-[.16em] text-[#A09A90]">ITEMS</p><h3 className="mt-1 text-sm font-semibold">商品明細</h3></div><span className="rounded-full bg-[#EEF5EF] px-3 py-1 text-[11px] font-semibold text-[#477154]">共 {itemCount} 件</span></div><div className="overflow-hidden rounded-xl border border-[#E8E4DE]"><div className="grid grid-cols-[62px_minmax(0,1fr)_64px_36px_68px] gap-2 bg-[#FBFAF8] px-3 py-2.5 text-[10px] font-semibold text-[#938D84]"><span>分類</span><span>商品名稱</span><span className="text-right">售價</span><span className="text-right">數量</span><span className="text-right">總計</span></div>{order.order_items.map((item) => <div key={item.id} className="grid grid-cols-[62px_minmax(0,1fr)_64px_36px_68px] gap-2 border-t border-[#F0EDE8] px-3 py-3 text-xs"><span className="text-[#6D675F]">{item.category}</span><span><b className="block text-xs">{item.product_name}</b></span><b className="text-right text-[10px]">{currency(item.unit_price)}</b><b className="text-right text-xs">× {item.quantity}</b><b className="text-right text-[10px]">{currency(item.unit_price * item.quantity)}</b></div>)}</div></div>

        <footer className="flex items-center justify-end gap-5 border-t border-[#E8E4DE] bg-[#FCFBF9] px-5 py-4 text-xs">{order.delivery_method === "賣貨便" && <p className="text-[#70695F]">運費：{order.delivery_fee ? currency(order.delivery_fee) : "免運"}</p>}<p className="font-semibold">訂單總額：{currency(order.total)}</p></footer></section>
      </section>
    </div>
  </main>;
}
