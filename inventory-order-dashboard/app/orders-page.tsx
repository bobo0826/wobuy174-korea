...
type StoredOrder = {
  id: string; order_number: string; order_date: string; status: Order["status"]; payment_method: string;
  reconciliation_status: string; delivery_method: string; delivery_fee: number; note: string;
...
const tones: Record<Tone, string> = {
  ...
  stone: "bg-[#F0EDE8] text-[#6F6960]",
};
const statusTone = (status: Order["status"]): Tone => status === "預購中" ? "orange" : status === "未出貨" ? "blue" : status === "已出貨" ? "green" : "stone";
...
function OrderDetail({ order, back, onUpdated, onDeleted, onInventoryChanged }: { order: Order; back: () => void; onUpdated: (order: Order) => void; onDeleted: (id: string) => void; onInventoryChanged: () => Promise<void> }) {
  ...
  const customerFields = [["客戶姓名", order.customer], ["LINE@名稱", order.lineName], ["電話", order.phone], ["地址", order.address]];
  const activities = [[order.createdAt, "建立訂單", "客戶與商品資訊已儲存。"], ["目前狀態", order.status, order.status === "已出貨" ? "庫存已扣除完成。" : "等待後續出貨處理。"], ["付款狀態", order.paymentStatus, `${order.payment} · ${order.delivery}`]];
  ...
  return <>
    <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><button onClick={back} className="text-sm font-semibold text-[#5E7665]">← 返回訂單管理</button><p className="mt-5 text-[11px] font-bold tracking-[.18em] text-[#A09A90]">ORDER DETAIL</p><h1 className="mt-2 text-[29px] font-semibold tracking-[-.055em] text-[#292824] sm:text-[33px]">訂單 {order.number}</h1><p className="mt-2 text-sm text-[#7B766E]">建立於 {order.createdAt}</p></div>
      <div className="flex flex-wrap items-center gap-2"><Pill tone={order.statusTone}>{order.status}</Pill><Pill tone={order.paymentStatus === "已付款" ? "green" : "orange"}>{order.paymentStatus}</Pill><a href={`/shipping-slip?orderId=${encodeURIComponent(order.id)}`} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center rounded-xl border border-[#DED9D1] bg-white px-3 text-sm font-semibold text-[#5E7665]">列印出貨單</a><button onClick={openEditor} className="inline-flex h-10 items-center justify-center rounded-xl border border-[#DED9D1] bg-white px-3 text-sm font-semibold text-[#5E7665]">修改訂單</button><button onClick={() => { void removeOrder(); }} disabled={saving} className="inline-flex h-10 items-center justify-center rounded-xl border border-[#F0D6C2] bg-white px-3 text-sm font-semibold text-[#A35F37] disabled:opacity-50">刪除訂單</button></div>
    </div>
...
export function Orders({ created, go, onInventoryChanged }: { created: boolean; go: (view: "create") => void; onInventoryChanged: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      ...
      } catch (reason) { if (active) setLoadError(reason instanceof Error ? reason.message : "無法讀取訂單資料。"); }
  ...
  const statuses = ["全部狀態", "預購中", "未出貨", "已出貨", "已取消"];
