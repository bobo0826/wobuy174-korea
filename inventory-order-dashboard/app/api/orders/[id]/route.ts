...
async function restoreCancelledOrder(id: string) {
  const supabase = getSupabaseAdmin();
  const { data: current, error: currentError } = await supabase
    .from("orders")
    .select("id, status")
    ...
    .single();
  if (currentError || !current) throw new Error("找不到訂單。");
  if (current.status !== "已取消") throw new Error("只有已取消的訂單可以復原。");

  const { data, error } = await supabase
    .from("orders")
    .update({ status: "預購中", updated_at: new Date().toISOString() })
    .eq("id", id)
    .select(orderSelect)
    .single();
  if (error) throw error;
